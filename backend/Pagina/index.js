...
const examSearchType = document.getElementById('examSearchType'); // Filtro de búsqueda
...
// Vista Examen (Preview/Resultado)
...
const examTypeInput = document.getElementById('examTypeInput'); // <--- NUEVA REFERENCIA IMPORTANTE
...
toggleSubjectFormBtn.addEventListener('click', () => { createSubjectForm.classList.toggle('hidden'); createCriterioForm.classList.add('hidden'); });
toggleCriterioFormBtn.addEventListener('click', () => { createCriterioForm.classList.toggle('hidden'); createSubjectForm.classList.add('hidden'); });
...
window.viewAttemptInModal = (intentoId) => {
    ...
    const notaColor = intento.nota >= 5 ? 'text-green-600' : 'text-red-600';
    modalTitle.innerHTML = `Revisión: ${nombreExamen} <span class="ml-2 text-sm bg-white px-2 py-1 rounded border ${notaColor}">Nota: ${intento.nota}</span>`;
            ...
            <div class="bg-white p-6 rounded-lg shadow border border-gray-200 ${colorClase} mb-6">
                ...
                <div class="space-y-2 mb-3">
                    ${preg.opciones.map(opcion => {
                        ...
                        if (marcado) estiloOpcion = esAcierto ? "border-green-500 bg-green-100 text-green-800 font-bold" : "border-red-500 bg-red-100 text-red-800 font-bold";
                        if (esLaCorrecta && !marcado) estiloOpcion = "border-green-500 bg-green-50 text-green-800 font-bold";
                        return `<div class="p-3 border rounded ${estiloOpcion} flex items-center"><span class="mr-2">${marcado ? '●' : '○'}</span>${escapeHTML(opcion)}${esLaCorrecta && !esAcierto ? '<span class="ml-auto text-xs bg-green-200 text-green-800 px-2 py-1 rounded">Solución</span>' : ''}</div>`;
                    }).join('')}
                </div>
            </div>
        `;
...
function displayAiResultsInline() {
    ...
    aiQuestionsList.innerHTML = currentAiQuestions.map((q, i) => `
        <div id="ai-item-${i}" class="bg-white p-4 rounded border border-purple-200 shadow-sm transition-all">
            <div id="ai-view-${i}"><p class="font-bold text-purple-900 mb-2"><span class="text-gray-400 mr-2">#${i+1}</span> ${escapeHTML(q.enunciado)}</p><p class="text-xs text-green-600 font-bold mb-2">Respuesta: ${escapeHTML(q.respuesta_correcta)}</p><button onclick="openAiEditor(${i})" class="text-xs bg-purple-100 text-purple-700 px-3 py-1 rounded hover:bg-purple-200 font-bold">✏️ Editar y Guardar</button><button onclick="removeAiItem(${i})" class="text-xs bg-red-50 text-red-600 px-3 py-1 rounded hover:bg-red-100 ml-2">🗑️ Descartar</button></div>
            <div id="ai-edit-${i}" class="hidden mt-2 border-t pt-3 border-purple-100">
                <div class="grid gap-3"><input type="text" id="edit-enunciado-${i}" value="${escapeHTML(q.enunciado)}" class="p-2 border rounded w-full text-sm font-bold text-gray-800"><div class="flex gap-2"><div class="w-1/2"><label class="text-xs text-gray-500 font-bold">Asignatura</label><select id="edit-asignatura-${i}" class="p-1 border rounded w-full text-sm bg-gray-50" onchange="loadCriteriaForInline(${i})"><option value="">-- Selecciona --</option></select></div><div class="w-1/2"><label class="text-xs text-gray-500 font-bold">Criterios</label><div id="edit-criterios-box-${i}" class="p-1 border rounded w-full h-20 overflow-y-auto text-xs bg-white"><p class="text-gray-400 italic">Elige asignatura...</p></div></div></div><div class="flex gap-2"><div class="w-1/3"><label class="text-xs text-gray-500 font-bold">Dificultad</label><input type="number" id="edit-dificultad-${i}" value="${q.dificultad || 1}" class="p-1 border rounded w-full text-sm"></div><div class="w-2/3"><label class="text-xs text-green-600 font-bold">Respuesta Correcta</label><input type="text" id="edit-correcta-${i}" value="${escapeHTML(q.respuesta_correcta)}" class="p-1 border-2 border-green-100 rounded w-full text-sm"></div></div><div id="edit-incorrectas-box-${i}" class="space-y-1">${q.opciones.filter(o => o !== q.respuesta_correcta).map(opt => `<input type="text" class="edit-incorrecta-${i} w-full p-1 border rounded text-xs text-gray-600" value="${escapeHTML(opt)}">`).join('')}</div><div class="flex justify-end gap-2 mt-2"><button onclick="closeAiEditor(${i})" class="text-xs text-gray-500 underline px-2">Cancelar</button><button onclick="saveAiQuestion(${i})" class="text-xs bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700 font-bold shadow">💾 Guardar en BD</button></div></div>
            </div>
        </div>`).join('');
}
window.openAiEditor = async (index) => { document.getElementById(`ai-view-${index}`).classList.add('hidden'); document.getElementById(`ai-edit-${index}`).classList.remove('hidden'); const select = document.getElementById(`edit-asignatura-${index}`); if (select.options.length <= 1) select.innerHTML = document.getElementById('newAsignatura').innerHTML; };
...
window.saveAiQuestion = async (index) => {
    ...
    try { const btn = event.target; btn.innerHTML = '...'; btn.disabled = true; const res = await fetch(URL_PREGUNTAS, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ enunciado, asignatura: asignaturaId, criterios_evaluacion: criteriosSelected, dificultad, respuesta_correcta: correcta, incorrect_options: incorrectas }) }); const data = await res.json(); if (data.success) { const itemDiv = document.getElementById(`ai-item-${index}`); itemDiv.classList.add('opacity-0', 'transform', 'scale-95'); setTimeout(() => itemDiv.remove(), 300); } else { alert(data.message); btn.innerHTML = '💾 Guardar en BD'; btn.disabled = false; } } catch (err) { alert('Error.'); }
};
...
async function saveExamToDB() {
    ...
    // Capturar valor del nuevo selector de tipo
    ...
    try { 
        const res = await fetch(`${URL_EXAMENES}/save`, {
            ...
            body:JSON.stringify({
                ...
                preguntasIds: currentExamData.preguntas.map(q=>q.id), 
                autor: author,
                tipo: tipo // Enviamos el tipo
            })
        }); 
        const d=await res.json(); 
        if(d.success) alert(`Examen guardado como ${tipo}`); else alert(d.message); 
    } catch(e){alert('Error');}
}
...
function displayExamsList(e) {
    ...
    examsListContainer.innerHTML = e.map(x=> {
        ...
        return `<div class="flex flex-col sm:flex-row justify-between items-center bg-white p-3 rounded shadow-sm border border-gray-200"><div class="mb-2 sm:mb-0"><h4 class="font-bold text-gray-800 text-sm flex items-center gap-2">${escapeHTML(x.nombre)} <span class="text-xs px-2 py-0.5 rounded border ${badgeColor}">${x.tipo||'PRACTICA'}</span></h4><p class="text-xs text-gray-500">${escapeHTML(x.asignatura?.nombre)}</p></div><div class="flex gap-2"><button onclick="takeExam('${x._id}')" class="text-green-600 text-xs font-bold border border-green-200 px-3 py-1 rounded hover:bg-green-50">✍️ Realizar</button><a href="${URL_EXAMENES}/${x._id}/pdf" target="_blank" class="text-indigo-600 text-xs font-bold border border-indigo-200 px-3 py-1 rounded hover:bg-indigo-50">📥 PDF</a></div></div>`;
    }).join('');
}
